import type { MetaFunction } from "react-router";
import { Outlet, useLoaderData } from "react-router";

import { requireManagerAccess } from "../auth/authorization.server";
import { accessLogoutUrl } from "../auth/identity";
import { AccessErrorPage } from "../components/AccessErrorPage";
import { AreaShell, type AreaNavItem } from "../components/AreaShell";
import { appEnvironmentFrom } from "../data/context.server";
import { site } from "../data/site";

export const meta: MetaFunction = () => [
  { title: `Manager — ${site.name}` },
  { name: "robots", content: "noindex, nofollow" },
];

const navItems: readonly AreaNavItem[] = [
  { to: "/manager", label: "Dashboard", end: true },
  { to: "/manager/diagnostics", label: "Diagnostics" },
  { to: "/manager/settings", label: "Settings" },
  { to: "/manager/maintenance", label: "Maintenance" },
];

/**
 * Server-side guard for the whole `/manager` area: manager role only. Child
 * routes repeat the guard in their own loaders — see the note in `app/routes.ts`.
 */
export async function loader({ request, context }: { request: Request; context: unknown }) {
  const user = await requireManagerAccess(request, context);
  return { user, logoutUrl: accessLogoutUrl(appEnvironmentFrom(context)) };
}

export default function ManagerLayout() {
  const { user, logoutUrl } = useLoaderData<typeof loader>();

  return (
    <AreaShell areaName="Manager" navItems={navItems} operator={user} logoutUrl={logoutUrl}>
      <Outlet />
    </AreaShell>
  );
}

export function ErrorBoundary({ error }: { error: unknown }) {
  return <AccessErrorPage error={error} />;
}
